...
//		.withUser("mike").password("mike123").roles("admin").and().passwordEncoder(NoOpPasswordEncoder.getInstance());
//		
//		auth.inMemoryAuthentication()
//		.withUser("neha").password("neha123").roles("client").and().passwordEncoder(NoOpPasswordEncoder.getInstance());
		auth.inMemoryAuthentication()
		.withUser("mike").password(this.passwordEncoder().encode("mike123")).roles("admin");
		
		auth.inMemoryAuthentication()
		.withUser("neha").password(this.passwordEncoder().encode("neha123")).roles("client");
